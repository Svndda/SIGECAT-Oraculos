...
const ROUTES: Record<string, RouteNode> = {
  '/': { label: 'Inicio' },
  '/employee-form': { label: 'Información del puesto', parent: '/' },
...
export default function Breadcrumbs() {
  ...
  return (
    <Box sx={{ px: { xs: 2, sm: 3 }, pt: 2 }}>
      <MuiBreadcrumbs separator={<NavigateNextIcon fontSize="small" sx={{ color: '#999' }} />}>
        {trail.map((step, index) => {
          const isLast = index === trail.length - 1;
          if (isLast) {
            return (
              <Typography key={step.path} sx={{ color: '#12457d', fontWeight: 600 }} variant="body2">
                {step.label}
              </Typography>
            );
          }
